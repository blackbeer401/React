...
class HomeComponent extends Component{
    // 값이 변경되면 화면이 갱신되는 Component 클래스의 아주 특별한 멤버변수가 있다 stat
    state = {
        image: newyork,
        //이미지들을 가진 배열
    ...
    render(){
        return (
            <div>
                ...
                {/* 3)public 폴더에는 index.html 이 바라보는 본인 폴더 위치임 */}
                {/*  public 폴더에 이밎 파일을 위치하고 사용해보기 */}
                <img src="./koala.jpg" alt="코알라 이미지" width={150}/>
                {/*  4. 웹 이미지 파일의 전체 url 경로 (절대경로) */}
                <img src="https://cdn.pixabay.com/photo/2019/09/17/02/20/jeju-4482313_1280.jpg" alt="제주 이미지"  width={150}/>
                {/* 이미지 파일을 사용하는 방법은 경오레 따라 다음 . 이중 권장은 src 폴더 내 asset 폴더 */}
                {/* public 폴더 안에는 아이콘 이미지들을 보통 배치하여 사용한다. */}

                {/* 버튼 클릭하여 이미지 변경하기  -- 화면 갱신에 영향을 주는 state 변수를 활용해야 한다. */}
                <img src={this.state.image} alt="뉴욕이미지" width={150} />
                <button onClick={this.changeImage}>change img</button>
 
                <hr />

                {/* 이미지  클릭시 이미지 변경*/}
                <img src={this.state.images[this.state.imgIndex]} alt="moana 이미지" width={200} onClick={this.changeImage2}/>
